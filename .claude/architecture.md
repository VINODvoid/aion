...
| Job Queue | BullMQ v5.71 + Redis 7.2 | Consequence scheduling, push notification dispatch |
| Database | PostgreSQL + Prisma v7 ORM | PrismaPg driver adapter, prisma.config.ts for URL |
...
│   │   │   └── migrations/      # Versioned migration files (not yet run)
│   │   ├── prisma.config.ts     # Prisma v7 config — DATABASE_URL lives here, not schema.prisma
│   │   └── src/
│   │       ├── index.ts         # Prisma client singleton (PrismaPg adapter) + re-exports
│   │       └── generated/       # Auto-generated by `prisma generate` — do not edit
│   │
│   ├── queue/
│   │   └── src/
│   │       ├── workers/         # BullMQ workers — consequence-worker.ts (idempotency guard built)
│   │       ├── jobs/            # Job schemas — consequence-job.ts (Zod schema + type)
│   │       ├── schedulers/      # EOD scheduler stub — per-timezone detection not yet implemented
│   │       └── clients/         # redis-client.ts (shared ioredis instance for BullMQ)
│   │
│   └── shared/
│       └── src/
│           ├── types/           # pact.ts — PactFrequency, PactStatus, ConsequenceType, etc.
│           ├── constants/       # xp.ts, consequence-tiers.ts, pact.ts — all game constants
│           └── utils/           # timezone.ts (4 fns), xp-calculator.ts (4 fns)
│
